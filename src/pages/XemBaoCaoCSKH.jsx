...
export default function XemBaoCaoCSKH() {
    ...
    // Selected personnel names for current user
    const [selectedPersonnelNames, setSelectedPersonnelNames] = useState(undefined); // undefined = chưa load, [] = đã load nhưng rỗng
    ...
    const [showPersonnelFilter, setShowPersonnelFilter] = useState(true);

    // --- Sync F3 Logic ---
    const handleSyncF3Report = async () => {
        if (!window.confirm(`Bạn có chắc chắn muốn đồng bộ TOÀN BỘ dữ liệu Báo Cáo CSKH từ F3?\n\n(Quá trình này có thể mất một chút thời gian nếu dữ liệu lớn)`)) return;
        ...
        try {
            ...
            if (firebaseData.length === 0) {
                alert("Không tìm thấy dữ liệu trên F3 (Báo cáo CSKH).");
                return;
            }
    ...
    const handleDeleteAll = async () => {
        if (!window.confirm("⚠️ CẢNH BÁO: Bạn có chắc chắn muốn XÓA TOÀN BỘ dữ liệu báo cáo CSKH không?\n\nHành động này KHÔNG THỂ khôi phục!")) return;
    ...
    useEffect(() => {
        const loadSelectedPersonnel = async () => {
            try {
                ...
                console.log('📝 [XemBaoCaoCSKH] Valid personnel names:', validNames);
                setSelectedPersonnelNames(validNames);
            } catch (error) {
                console.error('❌ [XemBaoCaoCSKH] Error loading selected personnel:', error);
                setSelectedPersonnelNames([]);
            }
        };
    ...
    useEffect(() => {
        const fetchGlobalOptions = async () => {
            try {
                ...
                // Bước 2: Load từ sales_reports với giới hạn để lấy unique values (chỉ cần một số lượng nhỏ để có đủ options)
                // Chỉ fetch 10000 records mới nhất để lấy unique values, không cần fetch tất cả
                ...
                console.log(`✅ Đã load ${allData.length} records từ sales_reports để lấy filter options`);
                ...
                const newOptions = {
                    ...
                    teams: globalTeams,
                    shifts: globalShifts,
                    personnel: [] // Sẽ được cập nhật sau khi fetch data
                };
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Wait for selectedPersonnelNames to be loaded (if not Admin)
            // Admin có thể xem tất cả, không cần filter
            if (!isAdmin && selectedPersonnelNames === undefined) {
                console.log('⏳ Waiting for selectedPersonnelNames to load...');
                return;
            }
            ...
            try {
                ...
                try {
                    ...
                    while (hasMore) {
                        ...
                        // Filter by selected_personnel if available and not Admin
                        if (!isAdmin && selectedPersonnelNames && selectedPersonnelNames.length > 0) {
                            ...
                            if (orConditions.length > 0) {
                                directQuery = directQuery.or(orConditions.join(','));
                            } else {
                                // No valid names, return empty result
                                directQuery = directQuery.eq('id', '00000000-0000-0000-0000-000000000000');
                            }
                        }
                        
                        directQuery = directQuery.order('date', { ascending: false }).range(from, to);
                        
                        const queryResult = await directQuery;
                        
                        if (queryResult.error) {
                            error = queryResult.error;
                            hasMore = false;
                        } else {
                            allData = allData.concat(queryResult.data || []);
                            hasMore = (queryResult.data?.length || 0) === pageSize;
                            page++;
                            
                            if (queryResult.count && allData.length >= queryResult.count) {
                                hasMore = false;
                            }
                        }
                    }
        ...
        const processFetchedData = (apiData, employeeData) => {
            ...
            if (idFromUrl) {
                ...
                if (!isManager && userName) {
                    ...
                    newPermissions.title = 'BÁO CÁO CSKH';
                }
            }
            ...
            // Filter by selected_personnel if available (only for non-Admin users)
            if (!isAdmin && selectedPersonnelNames !== undefined) {
                const beforeFilter = visibleData.length;
                if (selectedPersonnelNames.length === 0) {
                    // User has no selected_personnel, show nothing
                    console.log(`⚠️ No selected_personnel found for user: ${userEmail}, showing empty data`);
                    visibleData = [];
                } else {
                    visibleData = visibleData.filter(r => {
                        const reportName = (r.ten || '').trim();
                        if (!reportName) return false;
                        
                        // Check if report name matches any selected personnel name (case-insensitive)
                        const matches = selectedPersonnelNames.some(selectedName => {
                            const normalizedReportName = reportName.toLowerCase().trim();
                            const normalizedSelectedName = String(selectedName).toLowerCase().trim();
                            return normalizedReportName === normalizedSelectedName || 
                                   normalizedReportName.includes(normalizedSelectedName) ||
                                   normalizedSelectedName.includes(normalizedReportName);
                        });
                        
                        return matches;
                    });
                    console.log(`📋 Filtered by selected_personnel: ${visibleData.length} records (from ${beforeFilter} total), selectedPersonnelNames:`, selectedPersonnelNames);
                }
            } else if (isAdmin) {
                console.log(`👑 Admin user: Showing all data (${visibleData.length} records)`);
            } else {
                console.log(`⏳ Waiting for selectedPersonnelNames to load...`);
            }
            ...
            // Update personnel options from filtered data (only show personnel in selected_personnel)
            let uniquePersonnel = [...new Set(visibleData.map(r => r.ten).filter(Boolean))].sort();
            
            // If selectedPersonnelNames is available and not Admin, only show those personnel in the filter
            if (!isAdmin && selectedPersonnelNames !== undefined && selectedPersonnelNames.length > 0) {
                uniquePersonnel = uniquePersonnel.filter(name => {
                    const normalizedName = String(name).toLowerCase().trim();
                    return selectedPersonnelNames.some(selectedName => {
                        const normalizedSelectedName = String(selectedName).toLowerCase().trim();
                        return normalizedName === normalizedSelectedName || 
            ...
            // Initialize personnel filter with all personnel selected by default
            setFilters(prev => ({
                ...prev,
                personnel: uniquePersonnel
            }));
    ...
    const filteredData = useMemo(() => {
        ...
        const filtered = rawData.filter(r => {
            ...
            // Filter theo Nhân sự
            if (filters.personnel.length > 0 && !filters.personnel.includes(r.ten)) return false;
                ...
                <div className="sidebar">
                    ...
                    <h3 style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <button
                            ...
                            style={{
                                fontSize: '0.75em',
                                padding: '2px 8px',
                                background: showPersonnelFilter ? '#4A6E23' : '#f0f0f0',
                                color: showPersonnelFilter ? '#fff' : '#666',
                                border: 'none',
                                borderRadius: '3px',
                                cursor: 'pointer',
                                fontWeight: '500',
                    ...
                    {showPersonnelFilter && (
                        <>
                            <label>
                                <input type="checkbox"
                                    checked={filters.personnel.length === options.personnel.length && options.personnel.length > 0}
                                    onChange={(e) => handleSelectAll('personnel', e.target.checked)}
                                /> Tất cả
                            </label>
                            <div className="indent" style={{ maxHeight: '200px', overflowY: 'auto' }}>
                                {options.personnel.length === 0 ? (
                                    <div style={{ fontSize: '11px', color: '#999', padding: '5px' }}>Đang tải...</div>
                                ) : (
                                    options.personnel.map(person => (
                                        <label key={person} style={{ display: 'block', fontSize: '12px', marginBottom: '3px', cursor: 'pointer' }}>
                                            <input
                                                type="checkbox"
                                                checked={filters.personnel.includes(person)}
                                                onChange={(e) => handleFilterChange('personnel', person, e.target.checked)}
