...
export function useF3Data(filters, userRole, userEmail, userTeam = '') {
  ...
  const filteredF3Data = useMemo(() => {
    ...
    if (
      ...
      // Lấy thông tin user từ human_resources
      ...
      if (currentUser) {
        ...
        if (userName) {
          // Filter F3 data: chỉ hiển thị đơn hàng mà user là "Nhân viên Marketing" hoặc "Nhân viên Sale"
          filtered = filtered.filter((item) => {
            ...
            // Match by email prefix (username before @)
          ...
          // Không tìm thấy tên user, không hiển thị gì
          filtered = [];
        }
      } else {
        // Không tìm thấy user trong human_resources, không hiển thị gì
        filtered = [];
      }
    }
