-- =====================================================
-- Function lấy dữ liệu Báo Cáo Sale Tổng Hợp (V6 - Remove revenue_after_cancel_actual)
-- Updated: Removed "Doanh số sau hoàn hủy thực tế" column (will be calculated in frontend from orders table)
-- =====================================================
...
CREATE FUNCTION get_sales_analytics(
  p_start_date DATE,
  p_end_date DATE
)
RETURNS TABLE (
...
BEGIN
  ...
  SELECT 
    ...
    -- Cast date for return too, just in case
  ...
  -- FIX: Cast sr.date to DATE explicitly to handle TEXT columns correctly
  WHERE sr.date::DATE BETWEEN p_start_date AND p_end_date
  ORDER BY sr.date DESC, sr.name ASC;
END;
$$ LANGUAGE plpgsql;

SELECT 'Function get_sales_analytics V6 (Removed revenue_after_cancel_actual) updated!' as message;
