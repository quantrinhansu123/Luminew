-- =============================================================================
-- Rebuild kpis_mkt: schema cũ (ADS: so_mess, so_don, …) → cột KPI Sale / KPIs MKT
-- Bảng đang trống hoặc data chốt cũ không dùng được → DROP + CREATE lại.
-- =============================================================================

DROP TABLE IF EXISTS public.kpis_mkt CASCADE;
...
CREATE TABLE public.kpis_mkt (
  ...
  CONSTRAINT kpis_mkt_date_nv_product_market_unique
    UNIQUE (report_date, employee_name, product, market)
);

CREATE INDEX idx_kpis_mkt_report_date ON public.kpis_mkt (report_date);
CREATE INDEX idx_kpis_mkt_employee_name ON public.kpis_mkt (employee_name);
CREATE INDEX idx_kpis_mkt_team ON public.kpis_mkt (team);
CREATE INDEX idx_kpis_mkt_department ON public.kpis_mkt (department);
CREATE INDEX idx_kpis_mkt_product ON public.kpis_mkt (product);
CREATE INDEX idx_kpis_mkt_market ON public.kpis_mkt (market);
CREATE INDEX idx_kpis_mkt_date_team ON public.kpis_mkt (report_date, team);
