-- =============================================================================
-- kpis_mkt: lưu KPI MKT tổng hợp theo ngày × nhân viên (cùng shape kpi_sale)
-- Nguồn: orders (+ users HR, detail_reports CPQC). Chốt từ KPISale.html ?dept=MKT
-- Grain: 1 dòng = 1 nhân viên MKT / 1 ngày lên đơn
-- =============================================================================

CREATE TABLE IF NOT EXISTS public.kpis_mkt (
...
CREATE INDEX IF NOT EXISTS idx_kpis_mkt_product
  ON public.kpis_mkt (product);

CREATE INDEX IF NOT EXISTS idx_kpis_mkt_market
  ON public.kpis_mkt (market);
