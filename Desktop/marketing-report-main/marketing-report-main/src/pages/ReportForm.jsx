...
function ReportForm() {
  ...
  const [rndProducts, setRndProducts] = useState([]); // Sản phẩm test (SP Test) để loại bỏ

  // Load danh sách sản phẩm test từ system_settings
  useEffect(() => {
    const fetchRndProducts = async () => {
      try {
        ...
        if (data && data.settings && Array.isArray(data.settings.rndProducts)) {
          setRndProducts(data.settings.rndProducts);
          return;
        }
      } catch (err) {
        console.warn('Error fetching R&D products:', err);
      }
      ...
      try {
        ...
        if (s) {
          ...
          if (parsed.rndProducts && Array.isArray(parsed.rndProducts)) {
            setRndProducts(parsed.rndProducts);
          }
        }
      } catch (e) {
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        ...
        // Loại bỏ các sản phẩm test (SP Test) khỏi danh sách
        const filteredProducts = Array.from(productsSet).filter(p => !rndProducts.includes(p));

        setProductOptions(filteredProducts.sort((a, b) => a.localeCompare(b, 'vi')));
        setMarketOptions(Array.from(marketsSet).sort((a, b) => a.localeCompare(b, 'vi')));
      } catch (err) {
        console.error('Error fetching dropdown data:', err);
        // Loại bỏ sản phẩm test ngay cả khi fallback
        const fallbackProducts = ['Lumi Eyes', 'Lumi Nano', 'Lumi Skin'].filter(p => !rndProducts.includes(p));
