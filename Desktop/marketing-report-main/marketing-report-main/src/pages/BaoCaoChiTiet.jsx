...
// Các cột tự động ẩn mặc định (giống bên Sale)
...
function BaoCaoChiTiet() {
    ...
    const allAvailableColumns = useMemo(() => {
        ...
        const otherCols = Array.from(allKeys)
            .filter(key => !defaultColumns.includes(key) && !HIDDEN_COLUMNS.includes(key))
            .sort();

        const endCols = pinnedEndColumns.filter(col => allKeys.has(col) && !HIDDEN_COLUMNS.includes(col));
    ...
    useEffect(() => {
        if (Object.keys(visibleColumns).length > 0) {
            ...
            localStorage.setItem('baoCaoChiTiet_visibleColumns', JSON.stringify(toSave));
        }
    }, [visibleColumns]);
